...
*** [3.7.8] Data link (layer 2) addresses
    ...
    - before an IP packet can be sent over a wired or wireless network, it MUST BE ENCAPSULATE IN A DATA LINK FRAME, so it can be transmitted over the physical medium
